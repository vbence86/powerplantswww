var Universe = window["Universe"] || function(window, document, undefined) {
    var HRSController = function() {
        var hrs_container, hrs_start, hrs_step = 1280, hrs_steps = [], hrs_step_last, hrs_step_number = 9, hrs_offset, hrs_ended = false, hrs_current = 0, hrs_scrolling = false;
        ...
        function previousPledge() {
            if (hrs_scrolling) return;
            updateCurrentStep();
            if (hrs_current > 0) scrollToPledge(--hrs_current);
        }
    ...
    var HeroController = function() {
        ...
        return {
            init: function(HRSController) {
                ...
                hero = $("#hero");
                $(window).scroll(function() {
                    var scrollTop = $(this).scrollTop(), hrs_start = HRSController.getStartPos(), hrs_step_last = hrs_step_last = HRSController.getEndPos(), distance = scrollTop - hrs_start, frame = Math.round(distance / 75);
                    if (distance + hrs_start > hrs_step_last) return;
